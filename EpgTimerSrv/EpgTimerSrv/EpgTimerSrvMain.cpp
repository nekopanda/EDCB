	...
	case CMD_EPG_SRV_DEL_AUTO_ADD:
		{
			resParam->param = OLD_CMD_ERR;
			{
				...
				if( CopySearchKeyData(&oldItem, cmdParam) == TRUE){
					vector<DWORD> val;
					val.push_back((DWORD)oldItem.iAutoAddID);
...
int CEpgTimerSrvMain::LuaDelAutoAdd(lua_State* L)
{
	...
	if( lua_gettop(L) == 1 ){
		vector<DWORD> val;
		val.push_back((DWORD)lua_tointeger(L, -1));
		ws.sys->DelAutoAdd(val);
	}
	return 0;
}
	...
	if( lua_gettop(L) == 1 && lua_istable(L, -1) ){
		...
		if( lua_istable(L, -1) ){
			...
			if( lua_istable(L, -1) ){
				...
				bool modified = true;
				if (item.dataID == 0) {
					modified = ws.sys->AddAutoAdd(val);
				}
				else {
					modified = ws.sys->ChgAutoAdd(val);
				}
				if( modified ){
					lua_pushboolean(L, true);
					return 1;
